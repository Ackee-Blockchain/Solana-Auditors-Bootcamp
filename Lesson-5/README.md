...
- [Solana Runtime Policy](#solana-runtime-policy)
    - [Ownership and Account Modification](#ownership-and-account-modification)
    - [Immutability](#immutability)
    - [Zero Initialization](#zero-initialization)
    - [Balance and Transaction Consistency](#balance-and-transaction-consistency)
    - [Atomicity](#atomicity)
    - [Memory Management](#memory-management)
    - [Program Ownership and Spending](#program-ownership-and-spending)
    - [Rent](#rent)
...
## Ownership and Account Modification
- Only the owner program may modify the contents of an account. Upon assignment, the data vector is guaranteed to be zero.
- `CreateAccount` and `Assign` guarantee that when an account is assigned to the program, the account's data is zero initialized.
- The runtime guarantees that a program's code is the only code that can modify account data for the account it is assigned to.
- Only the owner of an account may assign a new owner.

## Immutability
- Once assigned to a program, an account cannot be reassigned.
- Transactions that assign an account to a program or allocate space must be signed by the account's private key, unless the account is being created by `CreateAccountWithSeed`, in which case no corresponding private key exists for the account's address/pubkey.
- Executable is one-way (false->true) and only the account owner may set it.

## Zero Initialization
- Upon assignment, the data vector is guaranteed to be zero.
- `CreateAccount` and `Assign` guarantee that when an account is assigned to a program, its data is zero initialized.

## Balance and Transaction Consistency
- Total balances across all accounts are equal before and after the execution of a transaction.
- After a transaction is executed, the balances of read-only accounts must remain unchanged.
- The runtime guarantees that the balances belonging to accounts are balanced before and after the transaction.
- Only the owner of an account may subtract its lamports.
- Any program account may add lamports to an account.

## Atomicity
- All instructions in a transaction are executed atomically. If one fails, all account modifications are discarded.
- The runtime guarantees that all instructions are executed successfully when a transaction is committed.

## Memory Management
- There is no dynamic memory allocation. Clients must use `CreateAccount` instructions to allocate memory before passing it to another program.
- The `CreateAccount` instruction can be composed into a single transaction with a call to the program itself.

## Program Ownership and Spending
- The runtime guarantees that a program can only spend lamports in accounts that are assigned to it.
